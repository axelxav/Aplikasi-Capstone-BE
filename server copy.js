...
app.post("/clearDatabase", async (req,res) => {
  const queries = [
    `UPDATE sensor_data SET has_arrived = false`,
    `UPDATE sensor_data SET has_finished = false`,
    `UPDATE sensor_data SET has_open = false`,
    `UPDATE sensor_data SET user_id = null`,
  ];
  ...
  try {
    ...
    return res
      .status(200)
      .json({message: "Sensor_data has been cleaned successfully!"});
  } catch (error) {
    await pool.query("ROLLBACK");
    console.error("Error cleaning sensor_data:", error);
    return res
      .status(500)
      .json({ message: "Internal server error while cleaning sensor_data." });
  }
});
